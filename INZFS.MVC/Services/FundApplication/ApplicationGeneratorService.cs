...
namespace INZFS.MVC.Services.FundApplication
{
    public class ApplicationGeneratorService : Controller, IApplicationGeneratorService
    {
        ...
        public async Task<IActionResult> GetSection(string pagename, string id, ITempDataDictionary temp)
        {
            ...
            if (string.IsNullOrEmpty(_httpContextAccessor.HttpContext.User.Identity.ApplicationNumber()))
            {
                ...
                var name = claimIdentity.Name;
                claimIdentity.AddClaim(new Claim("ApplicationNumber", content.ApplicationNumber));
            }
            ...
            {
                var applicationOverviewContentModel = GetApplicationOverviewContent(content, temp);
            ...
            {
                var sectionContentModel = GetSectionContent(content, currentSection, temp);
        ...
        private SectionContent GetSectionContent(ApplicationContent content, Section section, ITempDataDictionary temp)
        {
            ...
            sectionContentModel.HasErrors = temp.ContainsKey(TempDataKeys.ApplicationOverviewError) && (bool)temp.ContainsKey(TempDataKeys.ApplicationOverviewError);
        ...
        public ApplicationOverviewContent GetApplicationOverviewContent(ApplicationContent content, ITempDataDictionary temp)
        {
            ...
            {
                var sectionContentModel = GetSectionContent(content, section, temp);
            ...
            applicationOverviewContentModel.HasErrors = temp.ContainsKey(TempDataKeys.ApplicationOverviewError)
                                    && (bool)temp.ContainsKey(TempDataKeys.ApplicationOverviewError);
